...
/*
 * 문제 이름 : 성격 유형 검사하기
 * 문제 설명 : 나만의 카카오 성격 유형 검사지를 만들려고 합니다. 성격 유형 검사는 다음과 같은 4개 지표로 성격 유형을 구분합니다. 성격은 각 지표에서 두 유형 중 하나로 결정됩니다. 
 			1번 지표	라이언형(R), 튜브형(T)
			2번 지표	콘형(C), 프로도형(F)
			3번 지표	제이지형(J), 무지형(M)
			4번 지표	어피치형(A), 네오형(N)
 ...
 * 입출력 예 : ["AN", "CF", "MJ", "RT", "NA"]	[5, 3, 2, 7, 5]		"TCMA"
 * 입출력 예 : ["TR", "RT", "TR"]	[7, 1, 3]	"RCJA"
...
public class exam71 {
	...
	public String solution(String[] survey, int[] choices) {
        ...
        for(int i=0;i<choices.length;i++) {
            String surveyKey = String.valueOf(survey[i].charAt(0));
            
            if(choices[i] < 4) {
                map.replace(surveyKey, map.get(surveyKey) + 4 - choices[i]);
            }else if(choices[i] > 4) {
                map.replace(surveyKey, map.get(surveyKey) + 4 - choices[i]);
            }
        }
